...
Console::BaseDocbook - base class for Docbook generating commands

=head1 PUBLIC INTERFACE

=head2 WriteDocbookFile()
Writes a complete module documentation chapter.

    my $Success = $DocbookObject->WriteDocbookFile(
        Chapter          => $Chapter,                                    # the chapter text (in docbook format)
        TargetLocation   => '/Packages/MyPackage/doc/en/MyPackage.xml',
    );
...
sub WriteDocbookFile {
    my ( $Self, %Param ) = @_;

    # Check needed parameters.
    for my $Needed (qw(Chapter TargetLocation)) {
        if ( !$Param{$Needed} ) {
            print "Need $Needed:!";
            return;
        }
...
XML
    ...
    if ($@) {
        $Self->PrintError("There was an error in the output file XML structure: $@\n");
        return;
    }
...
From OTRS Kernel/System/Main.pm.

=head2 _DirectoryRead()
