...
if ( $OTRSMajorVersion >= 5 ) {
    $Config{RebuildConfigCommand} = "$InstallDir/bin/otrs.Console.pl Maint::Config::Rebuild";
    $Config{DeleteCacheCommand}   = "$InstallDir/bin/otrs.Console.pl Maint::Cache::Delete";
}
else {
